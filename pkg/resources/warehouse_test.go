...
func expectReadWarehouse(mock sqlmock.Sqlmock) {
	mock.ExpectExec(`USE WAREHOUSE ?`).WithArgs("good_name").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("SHOW WAREHOUSES LIKE ?").WithArgs("good_name").WillReturnResult(sqlmock.NewResult(1, 1))
