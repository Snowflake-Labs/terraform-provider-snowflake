...
func TestAcc_FailoverGroup(t *testing.T) {
	...
	randomCharacters2 := strings.ToUpper(acctest.RandStringFromCharSet(10, acctest.CharSetAlpha))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: failoverGroupBasicInterval(randomCharacters2, accountName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "name", fmt.Sprintf("tst-terraform-%s", randomCharacters2)),
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "object_types.#", "4"),
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "allowed_accounts.#", "1"),
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "allowed_databases.#", "1"),
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "allowed_integration_types.#", "1"),
					resource.TestCheckResourceAttr("snowflake_failover_group.fg", "replication_schedule.0.cron.0.interval", "10"),
				),
			},
		},
	})
