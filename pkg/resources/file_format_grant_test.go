...
func TestParseFileFormatGrantID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseFileFormatGrantID("test-db|PUBLIC|test-file-format|USAGE|true|role1,role2")
	r.NoError(err)
...
func TestParseFileFormatGrantEmojiID(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseFileFormatGrantID("test-db❄️PUBLIC❄️test-file-format❄️USAGE❄️true❄️role1,role2")
	r.NoError(err)
	r.Equal("test-db", grantID.DatabaseName)
	r.Equal("PUBLIC", grantID.SchemaName)
	r.Equal("test-file-format", grantID.ObjectName)
...
func TestParseFileFormatGrantOldID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseFileFormatGrantID("test-db|PUBLIC|test-file-format|USAGE|role1,role2|true")
	r.NoError(err)
	r.Equal("test-db", grantID.DatabaseName)
	r.Equal("PUBLIC", grantID.SchemaName)
	r.Equal("test-file-format", grantID.ObjectName)
