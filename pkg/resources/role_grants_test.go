...
func TestParseRoleGrantsID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseRoleGrantsID("test-role|role1,role2|user1,user2")
	r.NoError(err)
	...
	r.Equal(2, len(grantID.Users))
	r.Equal("user1", grantID.Users[0])
	r.Equal("user2", grantID.Users[1])
}

func TestParseRoleGrantsEmojiID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseRoleGrantsID("test-role❄️role1,role2❄️user1,user2")
	r.NoError(err)
	r.Equal("test-role", grantID.ObjectName)
	r.Equal(2, len(grantID.Roles))
	r.Equal("role1", grantID.Roles[0])
...
func TestParseRoleGrantsOldID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseRoleGrantsID("test-role||||role1,role2|false")
	r.NoError(err)
	r.Equal("test-role", grantID.ObjectName)
	r.Equal(2, len(grantID.Roles))
	r.Equal("role1", grantID.Roles[0])
