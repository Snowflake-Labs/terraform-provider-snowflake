...
func TestOnAllTableGrantCreate(t *testing.T) {
	a := assert.New(t)
	...
	WithMockDb(t, func(db *sql.DB, mock sqlmock.Sqlmock) {
		mock.ExpectExec(
			`^GRANT SELECT ON ALL TABLES IN SCHEMA "test-db"."PUBLIC" TO ROLE "test-role-1"$`,
		).WillReturnResult(sqlmock.NewResult(1, 1))
		mock.ExpectExec(
			`^GRANT SELECT ON ALL TABLES IN SCHEMA "test-db"."PUBLIC" TO ROLE "test-role-2"$`,
		).WillReturnResult(sqlmock.NewResult(1, 1))
		expectReadOnAllTableGrant(mock)
		err := resources.CreateTableGrant(d, db)
		a.NoError(err)
	})
}

func expectReadOnAllTableGrant(mock sqlmock.Sqlmock) {
	role1Rows := sqlmock.NewRows([]string{
		"created_on", "privilege", "granted_on", "name", "granted_to", "grantee_name", "grant_option",
	}).AddRow(
		time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC), "SELECT", "TABLE", "test-db.PUBLIC.<TABLE>", "ROLE", "test-role-1", false,
	)
	role2Rows := sqlmock.NewRows([]string{
		"created_on", "privilege", "granted_on", "name", "granted_to", "grantee_name", "grant_option",
	}).AddRow(
		time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC), "SELECT", "TABLE", "test-db.PUBLIC.<TABLE>", "ROLE", "test-role-2", false,
	)
	mock.ExpectQuery(`^SHOW GRANTS TO ROLE test-role-1$`).WillReturnRows(role1Rows)
	mock.ExpectQuery(`^SHOW GRANTS TO ROLE test-role-2$`).WillReturnRows(role2Rows)
}
