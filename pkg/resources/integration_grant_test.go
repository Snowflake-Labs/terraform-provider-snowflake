...
func TestParseIntegrationGrantID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseIntegrationGrantID("test-integration|IMPORTED PRIVILEGES|true|role1,role2")
	r.NoError(err)
...
func TestParseIntegrationGrantEmojiID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseIntegrationGrantID("test-integration❄️IMPORTED PRIVILEGES❄️true❄️role1,role2")
	r.NoError(err)
	r.Equal("test-integration", grantID.ObjectName)
	r.Equal("IMPORTED PRIVILEGES", grantID.Privilege)
	r.Equal(true, grantID.WithGrantOption)
...
func TestParseIntegrationGrantOldID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseIntegrationGrantID("test-integration|||IMPORTED PRIVILEGES|role1,role2|true")
	r.NoError(err)
	r.Equal("test-integration", grantID.ObjectName)
	r.Equal("IMPORTED PRIVILEGES", grantID.Privilege)
	r.Equal(true, grantID.WithGrantOption)
