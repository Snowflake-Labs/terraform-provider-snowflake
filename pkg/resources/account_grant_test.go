...
func TestParseAccountGrantID(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseAccountGrantID("MONITOR EXECUTION|true|role1,role2")
	r.NoError(err)
...
func TestParsAccountGrantEmojiID(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseAccountGrantID("MONITOR EXECUTION❄️true❄️role1,role2")
	r.NoError(err)
	r.Equal("MONITOR EXECUTION", grantID.Privilege)
	r.Equal(true, grantID.WithGrantOption)
	r.Equal(2, len(grantID.Roles))
...
func TestParseAccountGrantOldID(t *testing.T) {
	r := require.New(t)

	grantID, err := resources.ParseAccountGrantID("ACCOUNT|||MONITOR EXECUTION|role1,role2|true")
	r.NoError(err)
	r.Equal("MONITOR EXECUTION", grantID.Privilege)
	r.Equal(true, grantID.WithGrantOption)
	r.Equal(2, len(grantID.Roles))
