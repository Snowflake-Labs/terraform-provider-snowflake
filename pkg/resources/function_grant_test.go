...
func TestParseFunctionGrantID(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseFunctionGrantID("MY_DATABASE❄️MY_SCHEMA❄️MY_FUNCTION❄️❄️privilege_name❄️false❄️role1,role2❄️share1,share2")
	r.NoError(err)
...
func TestParseFunctionGrantIDWithArgs(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseFunctionGrantID("MY_DATABASE❄️MY_SCHEMA❄️MY_FUNCTION❄️string,string❄️privilege_name❄️false❄️role1,role2❄️share1,share2")
	r.NoError(err)
	r.Equal("MY_DATABASE", grantID.DatabaseName)
	r.Equal("MY_SCHEMA", grantID.SchemaName)
	r.Equal("MY_FUNCTION", grantID.ObjectName)
...
func TestParseFunctionGrantOldID(t *testing.T) {
	r := require.New(t)
	grantID, err := resources.ParseFunctionGrantID("MY_DATABASE|MY_SCHEMA|MY_FUNCTION()|privilege_name|role1,role2|false")
	r.NoError(err)
	r.Equal("MY_DATABASE", grantID.DatabaseName)
	r.Equal("MY_SCHEMA", grantID.SchemaName)
	r.Equal("MY_FUNCTION", grantID.ObjectName)
