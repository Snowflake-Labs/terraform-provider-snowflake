...
test:  ## run the tests (except sdk tests)
	CGO_ENABLED=1 go test -race -coverprofile=$(COVERAGE_REPORT_FILE) -covermode=atomic ./pkg/resources/...
	CGO_ENABLED=1 go test -race -coverprofile=$(COVERAGE_REPORT_FILE) -covermode=atomic ./pkg/provider/...
	CGO_ENABLED=1 go test -race -coverprofile=$(COVERAGE_REPORT_FILE) -covermode=atomic ./pkg/snowflake/...
.PHONY: test

test-acceptance: ## runs all tests, including the acceptance tests which create and destroys real resources
	SKIP_MANAGED_ACCOUNT_TEST=1 SKIP_EMAIL_INTEGRATION_TESTS=1 TF_ACC=1 go test -timeout 1200s -v -coverprofile=$(COVERAGE_REPORT_FILE) -covermode=atomic ./...
