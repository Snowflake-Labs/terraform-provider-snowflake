...
-> **Note** Currently `private_key` and `private_key_passphrase` are coupled and must be set in one source (both on Terraform side or both in TOML config, see https:///terraform-provider-snowflake/issues/3332). This will be fixed in the future.
