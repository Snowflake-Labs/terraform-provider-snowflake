...
-> **Note** In order to promote secondary_connection to primary_connection, resources need to be migrated manually. For guidance on removing and importing resources into the state check [resource migration](https://github.com/Snowflake-Labs/terraform-provider-snowflake/blob/main/docs/technical-documentation/resource_migration.md). Before importing the connection needs to be promoted in the Snowflake Worksheet using:
```
