...
-> **Note** In order to demote primary_connection to secondary_connection, resources need to be migrated manually. For guidance on removing and importing resources into the state check [resource migration](https://github.com/Snowflake-Labs/terraform-provider-snowflake/blob/main/docs/technical-documentation/resource_migration.md). Before impoting the connection needs to be recreated in Snowflake Worksheet using:
```
