...
[//]: # (TODO: account identifier automatically migrated, databases changed names in schema)
...
The output of **DESC DATABASE** is saved in the `describe_output` field and the output of **SHOW PARAMETERS IN DATABASE** in the `parameters` field inside `databases` entry.
